...
try
{
    ...
    Log.Information("TradeFlow Analytics — {Report} report | {From:yyyy-MM-dd} to {To:yyyy-MM-dd ET}",
        options.Report, options.From, options.To);

    // Reuse the same DbContext pattern as Worker and Api
    var connectionString = Environment.GetEnvironmentVariable("TRADEFLOW_CONNECTION_STRING")
        ?? "Host=localhost;Database=tradeflow;Username=tradeflow_user;Password=tradeflow_dev";

    var services = new ServiceCollection();

    services.AddLogging(b => b.AddSerilog());

    services.AddDbContext<TradeFlowDbContext>(o =>
    {
